...
const fn_mod = async (): Promise<void> => {
    try {
        const resp: AxiosResponse<ModResponse> = await axios.post('http://localhost:80/emp_mod.ajax', vo);
...
<tbody>
    <tr>
        <td>사번</td>
<td>
<input type="text" bind:value={vo.e_id} readonly/>
</td>
</tr>
<tr>
<td>이름</td>
...
<tr>
<td colspan="2">
<input type="button" value="저장" on:click={fn_mod} />
</td>
</tr>
</tbody>
</table>
